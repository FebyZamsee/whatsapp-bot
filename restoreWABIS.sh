...
DIRECTORY=/sdcard/WABIS

function ceknomer(){
file_path="/data/data/com.whatsapp.w4b/shared_prefs/com.whatsapp.registration.RegisterPhone.xml"
phone_number=$(sed -n 's/.*<string name="com.whatsapp.registration.RegisterPhone.phone_number">\([^<]*\)<\/string>.*/\1/p' "$file_path")
echo " >$Y Nomor ->$G $phone_number $N"
...
function wipeData(){
    ...
    pm clear --user 0 com.whatsapp.w4b >/dev/null
    sleep 1
}
function killApp(){
    am kill com.whatsapp >/dev/null
    am force-stop com.whatsapp >/dev/null
    am kill com.whatsapp.w4b >/dev/null
    sleep 0.5
